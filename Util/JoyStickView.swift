//EDITED: May 26, 2017
//MODIFIER: Steven Hurtado
//REASON: Simple view use case, instead of ImageView
 ...
 */
public typealias JoyStickViewMonitor = (_ direction: float2) -> ()
...
public final class JoyStickView: UIView {
    ...
    public var baseAlpha: CGFloat {
        get {
            return baseView.alpha
        }
        set {
            baseView.alpha = newValue
        }
    }
    ...
    //
    public private(set) var direction: float2 = float2.init()
    ...
    private func initialize() {
        ...
        baseView.alpha = 0.48
        baseView.backgroundColor = UIColor.FlatColor.Gray.IronGray
        
        addSubview(baseView)
        baseView.frame = bounds
        UIView.setRounded(view: baseView)
        UIView.addShadow(view: baseView)
        
        handleView = UIView(frame: frame)
        handleView.alpha = 0.64
        handleView.backgroundColor = UIColor.FlatColor.Gray.WhiteSmoke
        
        addSubview(handleView)
        handleView.frame = bounds.insetBy(dx: 0.15 * bounds.width, dy: 0.15 * bounds.height)
        UIView.setRounded(view: handleView)
        UIView.addShadow(view: handleView)
    ...
    private func updateLocation(location: CGPoint) {
        ...
        if movable {
            ...
            //
            handleView.center = bounds.mid + delta
        }
        else {
            ...
            if newDisplacement > 1.0 {
                ...
                handleView.frame.origin = CGPoint(x: x + bounds.midX - handleView.bounds.size.width / 2.0,
                                                       y: y + bounds.midY - handleView.bounds.size.height / 2.0)
            }
            else {
                handleView.center = bounds.mid + delta
            }
